// Ported from xAI Grok Build (xai-org/grok-build):
//   crates/codegen/xai-grok-markdown/src/mermaid.rs
// Copyright 2023-2026 SpaceXAI
// Licensed under the Apache License, Version 2.0
//
// Self-contained terminal renderer for Mermaid diagrams (Unicode box-drawing).
// Local adaptations for leaf: MermaidArt::is_fallback so the host can fall back
// to source display inside leaf's own mermaid frame.
...
pub(crate) struct MermaidArt {
    /// Per-span styles from the engine. Leaf currently paints diagrams with a
    /// single theme color via [`plain_lines`]; kept for a future colored path
    /// and for the engine's own tests.
    #[allow(dead_code)]
    pub styled_lines: Vec<Line<'static>>,
    pub plain_lines: Vec<String>,
    /// `true` when the source could not be laid out as a diagram (unsupported
    /// type, oversize canvas, or oversize width). Hosts that draw their own
    /// frame should treat this as a signal to show the raw source instead.
